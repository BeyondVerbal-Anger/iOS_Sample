...
NSString *const kRecording_URL = @"https://angdetector.beyondverbal.com/v4/recording/";
...
-(void)startSessionSuccess:(void (^)(NSData *data))success
{
    ...
    NSURLSessionDataTask *dataTask = [[NSURLSession sharedSession] dataTaskWithRequest:request
                                                                     completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
                                                                         NSError *err = nil;
         NSDictionary *responseDictionary = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingAllowFragments error:&err];
